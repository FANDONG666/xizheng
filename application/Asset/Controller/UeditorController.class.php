...
class UeditorController extends Controller {
	...
	//http://wangge.tao3w.com/index.php?g=Asset&m=Ueditor&a=ueditor_template_list
	public function ueditor_template_list()
	{
		$this->_header();
		$ueditor = M('ueditor')->order("listorder asc")->select();
		foreach ($ueditor as $k => $v){
			$ueditor[$k]['preHtml'] = $v['prehtml'];
			unset($ueditor[$k]['prehtml']);
		}
		echo json_encode($ueditor);
		die();
	}
	
	//http://wangge.tao3w.com/index.php?g=Asset&m=Ueditor&a=ueditor_template_del&id=900
	public function ueditor_template_del($id)
	{
		$this->_header();
		$where['id'] = intval($_REQUEST['id']);
		$delete = M('ueditor')->where($where)->delete();
	...
	//http://wangge.tao3w.com/index.php?g=Asset&m=Ueditor&a=ueditor_template_save&id=5&title=title5&prehtml=prehtml5&html=html5
	public function ueditor_template_save($id)
	{
		$this->_header();
		
		$where['id']     = intval($_REQUEST['id']);
		$data['title']   = $_REQUEST['title'];//I('param.title');
		$data['prehtml'] = $_REQUEST['preHtml'];//I('param.prehtml');
		$data['html']    = $_REQUEST['html'];//I('param.html');
		M('ueditor')->where($where)->save($data);
		$array['code']    = 0;
		$array['message'] = '成功';
		$array['data']    = intval($_REQUEST['id']);
		echo json_encode($array);
		die();
	}
	
	//http://wangge.tao3w.com/index.php?g=Asset&m=Ueditor&a=ueditor_template_add&title=title&prehtml=prehtml&html=html
	public function ueditor_template_add()
	{
		$this->_header();
