...
class AdminPostController extends AdminbaseController {
	...
	public function index(){
		...
		// 获取子分类树结构
        $parent_term = $_REQUEST['parent_term'];    // 菜单栏最高分类id
		$this->_getSubTree($parent_term);
		$this->display();
	}
	...
	private function _lists($where=array()){
		...
		$termsArr = sp_get_all_child_terms(intval($_REQUEST['term']));//benjamin by 2017-05-10
		array_unshift($termsArr, sp_get_term(intval($_REQUEST['term'])));//benjamin by 2017-05-11
		$inStr = select_to_in($termsArr,'term_id');//benjamin by 2017-05-10
		
		if(!empty($term_id)){
// 		    $where['b.term_id']=$term_id;
		    $where['b.term_id'] = array('in',$inStr);//benjamin by 2017-05-10
			$term=$this->terms_model->where(array('term_id'=>$term_id))->find();
			$this->assign("term",$term);
		}
	...
	// 获取文章分类树结构 
	protected function _getTermTree($term=array()){
		$result = $this->terms_model->order(array("listorder"=>"asc"))->select();
    ...
    /**
     * 获取子分类树结构 Sun 2017/05/10
     *
     * @param $parentTerm   菜单栏最高分类id   用于搜索表单里分类下拉列表的显示，需要显示菜单栏选中分类下的所有子分类
     */
    private function _getSubTree($parentTerm){
        $term_id=empty($_REQUEST['term'])?0:intval($_REQUEST['term']);
        $result = $this->terms_model->order(array("listorder"=>"asc"))->select();
        ...
        // 初始点击菜单栏的时候没有parent_term参数，但此时term就相当于parent_term
        if (!$parentTerm) {
            $parentTerm = $term_id;
        }
        $tree->init($array);
        $str="<option value='\$id' \$selected>\$spacer\$name</option>";
        $taxonomys = $tree->get_tree($parentTerm, $str);
        $this->assign("taxonomys", $taxonomys);
    }
