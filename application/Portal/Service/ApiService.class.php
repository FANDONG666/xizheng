...
class ApiService {
    ...
    public static function posts($tag,$where=array(),$pagesize=0,$pagetpl=''){
    	...
    	if (isset($tag['cid'])) {
    	    ...
    	    $inArr = array();
    	    foreach ($tag['cid'] as $k => $v){
    	    	$termsArr = sp_get_all_child_terms($v);//benjamin by 2017-05-10
    	    	array_unshift($termsArr, sp_get_term($v));
    	    	$inArr[] = select_to_in($termsArr,'term_id');//benjamin by 2017-05-10
    	    }
    	    $inStr = join(",", $inArr);
    	    
    		$where['term_relationships.term_id'] = array('in',$inStr);
